...
///
public class BallCutter : MillingCutter
{
		public BallCutter()
		{
		...
		public BallCutter(double d, double l)
		{
			diameter = d;
			Debug.Assert(diameter > 0.0);
			radius = d / 2.0;
			length = l;
			Debug.Assert(length > 0.0);
...
//ORIGINAL LINE: System.Tuple< double, double > singleEdgeDropCanonical(const Point& u1, const Point& u2) const
		protected new Tuple< double, double > singleEdgeDropCanonical(Point u1, Point u2)
		{
			...
			Point normal = new Point(u2.z - u1.z, -(u2.x - u1.x), 0); // (dz, -du) is a normal to the line
			normal.xyNormalize();
			if (normal.y < 0) // flip normal so it points upward
			{
				normal = -1 * normal;
			}
			Point cc = new Point(-s * normal.x, u1.y, 0); // from (0,u1.y,0) we go a distance -s in the normal direction
			cc.z_projectOntoEdge(u1, u2);
			double cl_z = cc.z + s * normal.y - radius;
			return new Tuple< double, double >(cc.x, cl_z);
		}
...
//C++ TO C# CONVERTER WARNING: 'const' methods are not available in C#:
//ORIGINAL LINE: bool calcCCandUpdateInterval(double t, const Point& p1, const Point& p2, const Fiber& f, Interval& i) const
...
//ORIGINAL LINE: double height(double r) const
		...
		{
			return radius - Math.Sqrt(ocl.GlobalMembers.square(radius) - ocl.GlobalMembers.square(r));
		}
//C++ TO C# CONVERTER WARNING: 'const' methods are not available in C#:
//ORIGINAL LINE: double width(double h) const
		protected new double width(double h)
		{
			return (h >= radius) ? radius : Math.Sqrt(ocl.GlobalMembers.square(radius) - ocl.GlobalMembers.square(radius - h));
		}
}
